...
In the current implementation the safety radius is \emph{class-dependent}, which is where the European ground-risk logic enters the algorithm rather than only the motivation. For uninvolved-person classes (the canonical \texttt{bike} family: \texttt{person}, \texttt{bicycle}, \texttt{biker}, and any unrecognized class by conservative default) the radius is derived from the SORA Ground Risk Buffer through its conservative 1:1 rule, by which the horizontal buffer kept to uninvolved third parties is at least the operating height \(h\) above ground:
\begin{equation}
R_s(\mathrm{person}) = \mathrm{clip}\!\left(\kappa\,h,\; R_{\min},\; R_{\max}\right),\quad \kappa=1,\; R_{\min}=\SI{15}{\meter},\; R_{\max}=\SI{40}{\meter}.
\end{equation}
Static assets and animals keep fixed geometric clearances, \(R_s(\mathrm{tower})=\SI{8}{\meter}\) and \(R_s(\mathrm{cow})=\SI{12}{\meter}\), the tower being smallest because inspection requires flying close to the asset. This turns safety into a hard, regulation-traceable constraint that grows with altitude like the SORA buffer, instead of a single hand-tuned penalty applied identically to a person, an animal, and a tower. At the validated cruise height \(h\approx\SI{23}{\meter}\) this gives \(R_s(\mathrm{person})=\SI{23}{\meter}>R_s(\mathrm{cow})>R_s(\mathrm{tower})\).
...
\begin{itemize}
    ...
    \item \textbf{SITL:} ArduPilot Software-In-The-Loop provides the simulated autopilot and vehicle dynamics.
    \item \textbf{MAVLINK Telemetry}: vehicle state (position, attitude, ground speed, mode) streamed from SITL to PORCE.
...
\subsection{Regulatory Operationalization and Traceability}
\label{subsec:reg}
A recurring weakness of inspection planners is \emph{obstacle homogeneity}: a tower, an animal, and an uninvolved person are inflated by the same margin, so the European distinction between an asset and a protected third party never reaches the planner. The proposed method removes that homogeneity at the single place where it changes behaviour, the class-dependent inflation radius $R_s(c)$ introduced above. Table~\ref{tab:classradius} lists the radii and Table~\ref{tab:traceability} maps each regulatory concept to the concrete mechanism and the repository artifact that implements it.
...
\begin{table}[t]
    ...
    \begin{tabular}{@{}lll@{}}
        \toprule
        \textbf{Class} & \textbf{$R_s$} & \textbf{Regulatory basis} \\
        \midrule
        Uninvolved person & $\mathrm{clip}(h,15,40)\,$m & SORA GRB, 1:1 rule \\
        Animal (cow) & \SI{12}{\meter} & Physical obstacle (fixed) \\
        Asset (tower) & \SI{8}{\meter} & Inspection target (fixed) \\
        Unknown & as person & Conservative default \\
        \bottomrule
    \end{tabular}
    \caption{Class-dependent safety radius $R_s(c)$. At cruise height $h\approx\SI{23}{\meter}$ the person radius is \SI{23}{\meter}, above the animal and asset clearances.}
    \label{tab:classradius}
\end{table}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{Imagenes/porce_grid_class_inflation.png}
\caption{Local A\(^\star\) grid with class-dependent inflation: an uninvolved person (\(R_s=\SI{23}{\meter}\) via the SORA 1:1 rule), a cow (\SI{12}{\meter}) and a tower (\SI{8}{\meter}) inflated by their own radius on the \SI{6}{\meter} grid, with the bounded A\(^\star\) route around them toward the mission waypoint.}
\label{fig:grid}
\end{figure}

Because the method is reactive rather than predictive, the preserved clearance is governed by a reaction-time budget. Let $v_c$ be the closing speed and $T_{\mathrm{act}}=T_{\mathrm{perc}}+\Delta T_r+T_{\mathrm{loop}}$ the total reaction latency (perception/publish, replan interval, control period). Under worst-case straight-line closure the realized minimum separation obeys $d_{\min}\gtrsim d_{\mathrm{det}}-v_c\,T_{\mathrm{act}}$, where $d_{\mathrm{det}}$ is the distance at first reliable detection. Preserving the hard radius, $d_{\min}\ge R_s$, gives the sufficient condition
\begin{equation}
d_{\mathrm{det}}\;\ge\;R_s+v_c\,T_{\mathrm{act}},
\label{eq:budget}
\end{equation}
which is itself perception-limited because a target of physical height $h_{\mathrm{obj}}$ is reliably detected only within $d_{\mathrm{det}}\le f_y\,h_{\mathrm{obj}}/p_{\min}$ (focal length $f_y$, minimum reliable pixel height $p_{\min}$). Equation~\eqref{eq:budget} makes the design levers explicit: a larger detection distance, a lower cruise speed near protected detections, or a tighter latency all widen the guaranteed margin, without invoking any motion model. The audited case in Table~\ref{tab:case} predates the class-dependent radius (it used a uniform \SI{12}{\meter} inflation); under $R_s(\mathrm{person})=\SI{23}{\meter}$ the same encounter is expected to clear by a wider margin, pending re-execution.
...
\begin{table*}[t]
    ...
    \begin{tabular}{@{}p{0.26\textwidth}p{0.40\textwidth}p{0.26\textwidth}@{}}
        \toprule
        \textbf{EASA / SORA concept} & \textbf{System mechanism} & \textbf{Repository artifact} \\
        \midrule
        Uninvolved person (Reg.\ (EU) 2019/947; SORA 2.5)~\cite{EU2019_947,JARUS_SORA_2_5_2024} & Civilian-like detections canonicalized into the \texttt{bike} family with the largest, dedicated radius; unknown defaults to it & \texttt{flight\_controller.py}: class canonicalization, \texttt{\_obs\_safety\_radius\_m} \\
        Ground Risk Buffer / 1:1 rule (SORA Annex F)~\cite{JARUS_SORA_2_5_2024} & Person radius $R_s=\mathrm{clip}(\kappa h,R_{\min},R_{\max})$, $\kappa=1$; grows with altitude like the buffer & \texttt{constants.py}, \texttt{\_obs\_safety\_radius\_m} \\
        Obstacle non-homogeneity (asset $\neq$ person) & Per-obstacle inflation $n_s(o)=\lceil R_s(c_o)/\delta\rceil$, so person $>$ animal $>$ asset & \texttt{porce\_manager.py}: \texttt{plan\_route} \\
        M1(C) ground observation~\cite{EASA_AMC_GM_SORA25_2025} & Onboard detection of the corridor triggers a bounded local bypass instead of overflight, and logs it & \texttt{vision\_system.py}; evasion branch and audit events \\
        No-overflight objective & A$^\star$ removes from the corridor every cell inside a protected region & \texttt{porce\_manager.py}: \texttt{plan\_route} \\
        Auditability / bounded behaviour~\cite{EASA_AI_Roadmap_2.0} & Deterministic graph search with a per-decision zero-trust audit trail usable as compliance evidence & \texttt{flight\_controller.py}: decision and route events \\
        \bottomrule
    \end{tabular}
    \caption{Regulatory traceability: each EASA/SORA concept mapped to the algorithmic mechanism and the repository artifact that implements it.}
    \label{tab:traceability}
\end{table*}

To make the objective measurable, an \emph{overflight-exposure event} at control step $t$ is any uninvolved-person track whose horizontal distance to the vehicle falls below its protection radius, $E(t)=\lvert\{o\in\mathcal{O}_t : c_o=\mathrm{person}\ \wedge\ \lVert p_t-p_{o}\rVert_2<R_s(c_o)\}\rvert$, with mission exposure $E_{\mathrm{tot}}=\sum_t \mathbf{1}[E(t)>0]$. The operational claim is that enabling class-dependent replanning drives $E_{\mathrm{tot}}$ toward zero relative to the asset-agnostic baseline, at the small path and time overhead reported in the ablation.
...
\begin{figure}[h!]
\centering
\includegraphics[width=\columnwidth]{Imagenes/Obstaculo_detectado_pero_fuera_seguridad.png}
...
\caption{Evasion Stage (zoom). Safety action}
\label{deteccion dentro del margen zoom}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{Audited case study:} one clean zero-trust run (\texttt{20260612\_233504}) with trajectory logs, PORCE events, and per-frame vision audit payloads.
\end{itemize}
...
The selected zero-trust maneuver comes from run \texttt{20260612\_233504}: the trigger-side detections are \texttt{biker} tracks, the human-adjacent obstacle family that most directly supports the no-overflight claim of the proposed method. Figure~\ref{fig:montage} shows the perception-to-planner chain at the last pre-trigger vision frame (frame~5728): the detector and tracker yield 15 published \texttt{biker} tracks, and the bounded planner subset formed for this trigger contains 16 obstacles. This reconstruction is auditable end to end because the route event serializes the exact planner obstacle identities (D3 audit patch).

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{Imagenes/porce_detection_montage.png}
\caption{Perception-to-planner chain at the pre-trigger frame: raw detector output, geo-projected tracks, and the bounded planner obstacle subset that conditions activation.}
\label{fig:montage}
\end{figure}
...
Figure~\ref{fig:trajectory} shows the geometric effect of the method in a local ENU frame centered on the trigger-side telemetry state. The background lattice matches the real \SI{6}{\meter} planner discretization, and the shaded cells are the inflated obstacle proxy obtained from the published trigger-side \texttt{biker} tracks at the configured \SI{12}{\meter} safety radius. The vehicle departs from the nominal mission corridor, executes a bounded bypass quantized on the real grid, and rejoins the original path toward the active waypoint.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{Imagenes/porce_case_trajectory.png}
\caption{Audited maneuver in a local ENU frame: nominal corridor, inflated obstacle proxy on the real \SI{6}{\meter} planner grid, and the executed bounded bypass.}
...
Figure~\ref{fig:timeseries} shows the time evolution of the maneuver. The dynamic reaction horizon stays above the clearance signal throughout, and the time-synchronized geometric distance to the concurrently tracked obstacles remains above both the hard \SI{12}{\meter} safety radius and the \SI{22}{\meter} failsafe threshold for the entire event, with a minimum of \SI{34.27}{\meter}. The failsafe therefore never has to engage, consistent with its role as an escalation parameter for replanning failure rather than a routine clearance margin.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{Imagenes/porce_case_timeseries.png}
\caption{Time series of the audited maneuver: dynamic reaction horizon, time-synchronized clearance to tracked obstacles, the \SI{12}{\meter} safety radius and the \SI{22}{\meter} failsafe threshold.}
...
Table~\ref{tab:case} summarizes the main metrics derived from the audited run. The method is triggered at \SI{35.11}{\meter}, inside the observed reaction horizon (up to \SI{52.73}{\meter}). It computes an 11-point local route around a 16-obstacle planner subset and completes the maneuver in \SI{19.48}{\second}. The evasion path length is \SI{59.17}{\meter}; the equivalent straight-line segment between the first and last evasion points is \SI{55.75}{\meter}, so the local safety maneuver introduces a detour of only \SI{3.42}{\meter}. Most importantly, the minimum time-synchronized separation to the concurrently tracked \texttt{biker} obstacles is \SI{34.27}{\meter}, i.e. \SI{22.27}{\meter} above the configured \SI{12}{\meter} safety radius and also above the \SI{22}{\meter} failsafe threshold. The maneuver thus sustains an explicit, auditable margin well above the protection radius while keeping the detour minimal.
