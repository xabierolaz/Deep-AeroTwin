...
In this section, the results of the experiments performed with the environment present in the $Deep\_Aero\_Twin$ repository using the proposed method are represented. The visual evaluation is organized into two reproducible cases: a static tower obstacle and a moving peloton/biker obstacle.

The first controlled experiment evaluates a static tower obstacle on the WP1--WP2 segment. The UAS starts from nominal waypoint tracking, receives a tower detection while the obstacle is still outside the dynamic reaction distance, and later triggers the local A* planner once the reaction condition is met. Figure~\ref{fig:static_tower_sequence} condenses this complete sequence in a common top-down frame: the occupancy grid is shown only after the planner has generated the local search space, while the last panel summarizes the executed deviation and recovery with respect to the planned path.

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{Imagenes/paper_figure_1_static_tower_sequence.png}
\caption{Static tower encounter. The sequence shows nominal navigation, obstacle detection without safety action, reaction-distance margins, local A* triggering, active evasion, and route recovery.}
\label{fig:static_tower_sequence}
\end{figure*}

The second experiment stresses the same decision chain with a moving obstacle represented as a peloton/biker target. The Unreal scene was configured with the paper moving-obstacle profile and executed through the same waypoint-driven runtime used by the pipeline. Figure~\ref{fig:moving_peloton_sequence} combines real Unreal evidence from the validated flight with the audited planning logs: panel 2A shows only the current scripted cyclist peloton inside Unreal, panel 2B overlays the YOLO track boxes on the same Unreal view, panel 2C shows the detected moving track around the trigger instant, and panel 2D shows the resulting evasion response against the moving-obstacle centroid.

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{Imagenes/paper_figure_2_moving_peloton_sequence.png}
\caption{Moving-obstacle peloton encounter. The figure combines real Unreal frames of the current cyclist peloton and YOLO detections with logged biker tracks, peloton centroid evolution, and the active evasion segment generated by the UAS.}
\label{fig:moving_peloton_sequence}
\end{figure*}
...
The visual materials discussed in the paper were generated directly from repository logs by the scripts in \texttt{figuras\_paper\_unreal\_generadas/}. The Unreal evidence is copied into the LaTeX \texttt{Imagenes/} folder from validated flight captures, while the validation diagrams are rebuilt from the audited telemetry, vision, and planner logs.

\section{Experimental Validation of the Proposed Method}

\subsection{Controlled E2E Ablation}

The controlled E2E ablation is reconstructed from the campaign \texttt{20260612\_174114}, with ten runs per scenario. Figure~\ref{fig:e2e_ablation} overlays representative trajectories from the four operating modes. The two scenarios without detections are practically identical regardless of whether the replanning module is enabled, which confirms that the planner remains dormant when no conflict enters the system. The \texttt{replanner off + detections} scenario also follows the nominal corridor, while \texttt{replanner on + detections} introduces a visible local detour before rejoining the original path.


\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{Imagenes/paper_figure_3_e2e_ablation.png}
\caption{Controlled E2E ablation reconstructed from campaign \texttt{20260612\_174114}. The full-mission overlay shows that all scenarios complete the same inspection corridor, while the conflict-window inset isolates the behavioral specificity of the proposed method: only the combination of detections and enabled replanning produces a local detour around the injected \texttt{biker} obstacle.}
\label{fig:e2e_ablation}
\end{figure*}

Table~\ref{tab:e2e} quantifies this result. The mission always completes, and the overhead introduced by the avoidance behavior is small: relative to \texttt{replanner off + detections}, enabling the method under detections adds \SI{8.3}{\meter} to the mean path and approximately \SI{2}{\second} to the total mission.
...
\begin{table}[t]
    ...
    \begin{tabular}{@{}p{0.43\columnwidth} S[table-format=3.0] S[table-format=4.1] c@{}}
        ...
        \midrule
        replanner on + detections & 207 & 1463.9 & Yes \\
        replanner off + detections & 205 & 1455.5 & Yes \\
        replanner on + no detections & 206 & 1455.8 & Yes \\
        replanner off + no detections & 207 & 1455.7 & Yes \\
        \bottomrule
    \end{tabular}
...
The selected zero-trust maneuver comes from run \texttt{20260220\_092802} and is intentionally stricter than the earlier tower/cattle-like cases because the trigger-side detections are \texttt{biker} tracks, i.e. the human-adjacent obstacle family that most directly supports the no-overflight claim of the proposed method. Figure~\ref{fig:audited_evasion_validation} reconstructs the perception-to-planner chain from the last pre-trigger vision frame (frame 5053 at \(t=-\SI{0.095}{\second}\)): 300 raw detector boxes are reduced to 233 projected detections, then to four published \texttt{biker} tracks, of which the three nearest are used as the auditable proxy of the planner subset. This is the strongest zero-trust reconstruction possible from repository artifacts because the route event serializes the planner count but not the obstacle identities.


Panel 4B shows the geometric effect of the method in a local ENU frame centered on the trigger-side telemetry state. The background lattice matches the real \SI{6}{\meter} planner discretization, while the shaded occupied cells correspond to the auditable proxy obtained by inflating the three nearest published trigger-side \texttt{biker} tracks with the configured \SI{12}{\meter} safety radius. Two of those tracks, at 42.0 and \SI{42.1}{\meter}, quantize to the same seed cell, so the merged occupied block is a genuine property of the planner discretization rather than a drawing artifact. The vehicle then departs from the nominal mission corridor, executes a bounded bypass, and rejoins the original path before continuing toward WP9.


Panel 4C complements the map with the corresponding time series. The reaction horizon remains comfortably above the clearance signal throughout the event, while the reconstructed geometric distance to the trigger-side proxy tracks never crosses the hard \SI{12}{\meter} safety radius. It does, however, fall below the \SI{22}{\meter} failsafe threshold during part of the maneuver, which is consistent with the role of that threshold as an escalation parameter for replanning failure rather than a guaranteed obstacle-clearance margin.

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{Imagenes/paper_figure_4_audited_collision_evasion.png}
\caption{Audited collision-evasion maneuver from run \texttt{20260220\_092802}. Panel 4A traces the logged perception reduction from raw boxes to planner-proxy tracks, panel 4B maps the three trigger-side \texttt{biker} proxies and inflated occupied cells in the real \SI{6}{\meter} planner grid, and panel 4C verifies that the executed evasion remains above the hard \SI{12}{\meter} safety radius while operating close to the configured margin.}
\label{fig:audited_evasion_validation}
\end{figure*}
...
\IfFileExists{bibliografia/referencias_revision_porce.bib}{%
