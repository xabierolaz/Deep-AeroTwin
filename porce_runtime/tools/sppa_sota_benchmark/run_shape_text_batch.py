...
from bench_common import ROOT, configure_torch_vram_cap, emit, gpu_snapshot, mesh_stats, read_objects, torch_peak
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark Shap-E text-to-3D with one resident model load.")
    ...
    parser.add_argument("--karras-steps", type=int, default=16)
    ...
    emit(
        "SPPA_BENCH_MODEL",
        {
            "model": f"shap_e_text_k{args.karras_steps}",
            "status": "loaded",
            "load_sec": load_sec,
            "device": str(device),
    ...
    for item in objects:
        ...
        try:
            ...
            latents = sample_latents(
                ...
                diffusion=diffusion,
                guidance_scale=args.guidance_scale,
                model_kwargs={"texts": [prompt] * args.batch_size},
                progress=False,
                clip_denoised=True,
                use_fp16=args.use_fp16,
                use_karras=True,
