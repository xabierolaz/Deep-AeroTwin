...
def path_value(payload: dict[str, Any], dotted_path: str) -> Any:
    current: Any = payload
    for part in dotted_path.split("."):
        if not isinstance(current, dict) or part not in current:
            return None
...
def validate(payload: dict[str, Any], schema: dict[str, Any]) -> list[str]:
    ...
    conditional_errors = []
    evidence_sources = path_value(payload, "evidence.evidence_sources")
    evidence_sources = evidence_sources if isinstance(evidence_sources, list) else []
    for rule in schema.get("x-requiredWhenEvidenceSource", []):
        source = rule.get("source") if isinstance(rule, dict) else None
        if source not in evidence_sources:
            continue
        for path in rule.get("requiredPaths", []):
            if not path_exists(payload, path):
                conditional_errors.append(f"{source}:{path}:missing")
        for path, expected in (rule.get("equals") or {}).items():
            actual = path_value(payload, path)
            if actual != expected:
                conditional_errors.append(f"{source}:{path}!={expected}")
    return missing + const_errors + conditional_errors
