...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark SPAR3D with one resident model load.")
    ...
    for item in objects:
        ...
        try:
            ...
            points = glob_dict.get("point_clouds", [None])[0]
            if points is not None:
                points.export(out / f"{label}_points.ply")
            payload = {
                "model": "spar3d_warm",
                "label": label,
                "prompt": item["prompt"],
