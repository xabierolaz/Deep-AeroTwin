#!/bin/bash
exec > /mnt/d/Deep-AeroTwin-UE57-Test/tmp/cmp_tierra.log 2>&1
for i in $(seq 1 50); do
  ls /mnt/d/Deep-AeroTwin-UE57-Test/tmp/pipeline/vace_tierra/*.mp4 >/dev/null 2>&1 && break
  sleep 10
done
sleep 5
...
OLV=g1("/mnt/d/Deep-AeroTwin-UE57-Test/tmp/pipeline/vace_olive/*.mp4")
...
for fr,tag in [(20,"a"),(40,"b")]:
    cols=[("ORIGINAL Cesium",grab(ORIG,fr)),("VACE OLIVAR",grab(OLV,int(fr/191*49))),
          ("VACE TIERRA",grab(TIE,int(fr/191*49))),("FlashVSR",grab(FVSR,fr))]
    ...
    cv2.imwrite(f"/mnt/d/Deep-AeroTwin-UE57-Test/tmp/TIERRA_compare_{tag}.png",np.vstack([hdr,row]));print("wrote",tag)
PY
cp /mnt/d/Deep-AeroTwin-UE57-Test/tmp/TIERRA_compare_*.png '/mnt/c/Users/xabie/AppData/Roaming/Claude/local-agent-mode-sessions/3682cacb-9d71-4e50-8c63-b61d94a8e6ca/cdbbf262-19f2-47fc-b740-e9d24eed93a4/local_2595f30d-c4ba-45ce-9e18-016f151a053c/outputs/' 2>/dev/null
echo CMPTIERRA_DONE
