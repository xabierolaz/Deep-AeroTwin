...
function _fail([string]$msg, [int]$code = 1) {
  Write-Host "[b_launch_tabs] ERROR: $msg" -ForegroundColor Red
  exit $code
}
...
if (-not $wt) {
  Write-Host "[b_launch_tabs] WARN: Windows Terminal (wt.exe) not available or not invocable." -ForegroundColor Yellow
}
...
if (Test-Path $venvActivate) {
  ...
  Write-Host "[b_launch_tabs] WARN: venv not found at: $venvActivate (using system python)"
}
...
$eyesCmd   = "set PORCE_SYSTEM_MODE=REAL_TWIN && " + (_cdPipe "python -u vision_system.py 2>&1 | python tee.py --prefix `"$eyesPrefix`" --cap-lines $teeCapLines")
...
function _start_fallback_tab([string]$title, [string]$cmd) {
  ...
  try {
    $safeTitle = if ([string]::IsNullOrWhiteSpace($safeTitle)) { "PORCE-B" } else { $safeTitle }
    Write-Host "[b_launch_tabs] INFO: launching fallback tab '$safeTitle'."
    $tmpName = "porce_b_tab_" + [System.Guid]::NewGuid().ToString("N") + ".bat"
    ...
    Write-Host "[b_launch_tabs] WARN: fallback tab creation failed for '$safeTitle': $($_.Exception.Message)" -ForegroundColor Yellow
    return $false
  }
}
...
function _start_with_tabs() {
  ...
  try {
    ...
    _start_wt_tab "last" "BRAIN (REAL_TWIN)" $brainCmd
    _start_wt_tab "last" "EYES (REAL_TWIN VIDEO)" $eyesCmd
    _start_wt_tab "last" "VIZ RECORDER" $vizCmd
    return $true
  } catch {
    Write-Host "[b_launch_tabs] WARN: WT launch failed: $($_.Exception.Message)" -ForegroundColor Yellow
    return $false
  }
}

if (-not (_start_with_tabs)) {
  Write-Host "[b_launch_tabs] WARN: Falling back to cmd tabs." -ForegroundColor Yellow
  _start_fallback_tab "MASTER LOG" $masterCmd
  _start_fallback_tab "BRAIN (REAL_TWIN)" $brainCmd
  _start_fallback_tab "EYES (REAL_TWIN VIDEO)" $eyesCmd
