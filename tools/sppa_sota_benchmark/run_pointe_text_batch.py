...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark Point-E text-to-3D with one resident model load.")
    parser.add_argument("--objects-csv", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--vram-limit-gb", type=float, default=None)
    parser.add_argument("--device", default="cuda")
    parser.add_argument("--base-name", default="base40M-textvec")
    ...
    sampler = PointCloudSampler(
        ...
        aux_channels=["R", "G", "B"],
        guidance_scale=[args.guidance_scale, 0.0],
    ...
    emit(
        "SPPA_BENCH_MODEL",
        {
            "model": "point_e_text_sdf32" if args.mesh else "point_e_text_pointcloud",
            "status": "loaded",
            "load_sec": load_sec,
            "device": str(device),
    ...
    for item in objects:
        ...
        try:
            ...
            pc_npz_path = out / f"{label}_points.npz"
            pc_ply_path = out / f"{label}_points.ply"
            start = time.perf_counter()
            pointcloud.save(str(pc_npz_path))
            ...
            payload = {
                ...
                "pointcloud_npz_path": str(pc_npz_path.relative_to(ROOT)).replace("\\", "/"),
                "pointcloud_ply_path": str(pc_ply_path.relative_to(ROOT)).replace("\\", "/"),
