...
OUT = ROOT / "paper_semantic_proxy_3d" / "figures" / "sppa_language_to_parts_to_3d_v17.png"
...
def dim_arrow(ax, x0, x1, y, text, *, color=None, fontsize=4.6):
    color = color or COL["ink"]
    ax.add_patch(
        FancyArrowPatch(
            (x0, y), (x1, y),
            arrowstyle="<|-|>", mutation_scale=9, linewidth=0.7, color=color,
        )
    )
    ax.text((x0 + x1) / 2, y + 0.06, text, fontsize=fontsize, ha="center", va="bottom", color=color)


def truck_sealed(ax, x0, y0, cargo_u, cab_u, n_cargo_wheels):
    """Truck at sealed invariance proportions: variable cargo, bounded cab,
    fixed-radius wheels (n under cargo + 2 under cab)."""
    body_y = y0 + 0.52
    box3d(ax, x0, body_y, cargo_u, 0.40, COL["cargo"], depth=0.05, top="#f5d090")
    cab_x = x0 + cargo_u + 0.06
    box3d(ax, cab_x, body_y, cab_u, 0.52, COL["cab"], depth=0.045)
    ax.add_patch(
        Rectangle(
            (cab_x + cab_u * 0.36, body_y + 0.30), 0.10, 0.10,
            facecolor=COL["window"], edgecolor=COL["ink"], linewidth=0.3,
        )
    )
    total_w = cargo_u + 0.06 + cab_u
    ax.add_patch(
        Rectangle(
            (x0 + 0.02, body_y - 0.10), total_w, 0.06,
            facecolor=COL["chassis"], edgecolor=COL["ink"], linewidth=0.36,
        )
    )
    for i in range(n_cargo_wheels):
        wx = x0 + 0.16 + (cargo_u - 0.32) * (i / max(n_cargo_wheels - 1, 1))
        wheel(ax, wx, y0 + 0.20, 0.055)
    for wx in (cab_x + cab_u * 0.30, cab_x + cab_u * 0.72):
        wheel(ax, wx, y0 + 0.20, 0.055)
    return total_w
...
def render() -> None:
    ...
    # Sealed proportions from the parametric part-invariance check
    # (experiments/sppa_scale_variants/20260703_parametric_part_invariance_after_scheduler_policy.json):
    # short 5.2x2.3x2.7 m, cargo 2.263 m, 6 tires; long 8.2x2.3x2.7 m,
    # cargo 5.188 m (+2.925), 8 tires; cab/tire scale delta = 0.0.
    rounded(ax, 0.12, 0.30, 11.72, 2.42, "white", lw=0.95)
    ax.text(0.38, 2.50, "Visual consequence: the part that carries the evidence changes; other parts keep their role priors", fontsize=6.4, fontweight="bold", ha="left")

    ax.text(0.60, 2.06, "primitive parts", fontsize=5.45, fontweight="bold", ha="left")
    box3d(ax, 0.62, 1.56, 0.86, 0.20, COL["cargo"], depth=0.045, top="#f5d090")
    ax.text(0.64, 1.38, "cargo (variable)", fontsize=4.25, color=COL["accent"], ha="left")
    box3d(ax, 1.76, 1.46, 0.30, 0.45, COL["cab"], depth=0.042)
    ax.text(1.70, 1.29, "cab", fontsize=4.25, color=COL["cab"], ha="left")
    ax.add_patch(Rectangle((0.62, 1.05), 1.52, 0.050, facecolor=COL["chassis"], edgecolor=COL["ink"], linewidth=0.32))
    for wx in [0.74, 1.16, 1.80, 2.05]:
        wheel(ax, wx, 0.81, 0.043)
    ax.text(0.70, 0.56, "wheel radius fixed", fontsize=4.25, color=COL["ink"], ha="left")

    S = 0.53  # drawing units per metre
    short_cargo_u = 2.26308 * S
    long_cargo_u = 5.18808 * S
    cab_u = 1.656 * S

    arrow(ax, (2.42, 1.30), (2.98, 1.30), lw=0.9, scale=12)
    ax.text(3.15, 2.06, "short truck", fontsize=5.6, fontweight="bold", ha="left")
    truck_sealed(ax, 3.15, 0.72, short_cargo_u, cab_u, 4)
    dim_arrow(ax, 3.15, 3.15 + short_cargo_u, 1.84, "cargo 2.26 m", color=COL["accent"])
    ax.text(3.15, 0.40, "5.2 x 2.3 x 2.7 m  -  6 tires", fontsize=4.4, color=COL["muted"], ha="left")

    arrow(ax, (5.46, 1.30), (6.02, 1.30), lw=0.9, scale=12)
    ax.text(6.19, 2.06, "long truck", fontsize=5.6, fontweight="bold", ha="left")
    truck_sealed(ax, 6.19, 0.72, long_cargo_u, cab_u, 6)
    dim_arrow(ax, 6.19, 6.19 + long_cargo_u, 1.84, "cargo 5.19 m (+2.93 m)", color=COL["accent"])
    ax.text(6.19, 0.40, "8.2 x 2.3 x 2.7 m (same W x H)  -  8 tires", fontsize=4.4, color=COL["muted"], ha="left")

    chips = ["cab delta = 0.0", "tire delta = 0.0", "tires 6 -> 8", "cargo +2.93 m"]
    for i, c in enumerate(chips):
        ax.text(10.15, 1.98 - 0.36 * i, c, fontsize=4.5, fontweight="bold", color=COL["ink"], ha="left")
    ax.text(10.15, 0.44, "sealed invariance check", fontsize=3.9, color=COL["muted"], ha="left")
