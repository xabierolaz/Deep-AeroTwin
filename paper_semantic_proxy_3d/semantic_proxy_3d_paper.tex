...
\caption{The twin-delta mission (conceptual illustration; no new
measurements). (a)~The real world now contains an intruder tower: a real
flight photo recorded for this paper (the dashed box is the measured
YOLOE-26s detection of Section~\ref{sec:deployment-summary}).
...
evaluation (Section~\ref{sec:external-sanity}). Real-image probes (a road
cyclist, a mountain electrical tower, a mountain tractor, a tractor with
trailer, and two real flight photos of a tower and a tractor recorded for
...
\(n{=}52\) that interval resolves only large effects: it neither confirms
transfer of the margin nor excludes a small residual advantage. Second,
...
payload---the hull's coverage advantage is the regime the frozen part-query
task prices: coverage without selection (\(0.087\)--\(0.111\) versus
\(0.434\) query F1; Section~\ref{sec:ablations}). Third, the failure modes are informative and consistent with
...
Figure~\ref{fig:real-flight-probes} runs the full chain end to end on two
real flight photos recorded for this paper (2026-07-21). The tower photo
...
\(5.60\times5.60\times28.00\)~m and the builder emits a 396-triangle lattice
...
\(7.1\times6.0\)~m footprint is fused to \(4.75\times2.50\times2.60\)~m and a
576-triangle proxy. On the same crops and workstation, the measured neural
generators degrade on these aerial views: TripoSR (6~GB cap) returns
26{,}836- and 39{,}700-triangle meshes at \(0.49\)/\(0.09\)~s inference and
\(1.9\)~GB peak VRAM, but as amorphous blobs without tower or tractor
structure, and Hunyuan3D-2mini-turbo fails with no surface on both crops (5
and 20 inference steps), while a ground-level control photo works
(Figure~\ref{fig:real-flight-probes}d; per-case values in RP). Telemetry
for these replays is declared assumed flight
(\(45\)/\(35\)~m AGL, nadir camera), not measured; no 3D ground truth
exists for the photos, so this fidelity reading is qualitative, not an IoU
ranking. The four earlier archived probes (biker, mountain
tower, mountain tractor, tractor-trailer), the probe grid, per-probe
evidence, replay audits, and input-mode ablations are retained in RP.
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/fig_real_flight_probes.png}
\caption{Real flight-photo probes end to end (exploratory, no 3D ground
truth; declared assumed-flight telemetry). Rows: tower and tractor photos
recorded for this paper. (a)~YOLOE-26s evidence (bbox, native mask,
confidence). (b)~Geo-projected mask footprint (dashed red) against the
gated metric dims (blue): the tower's \(39.9\times17.5\)~m footprint is
clamped to \(5.60\times5.60\times28.00\)~m by the family aspect prior; the
tractor's \(7.1\times6.0\)~m is fused to \(4.75\times2.50\times2.60\)~m.
(c)~The compiled runtime proxies (396 and 576 triangles): the detector
mislabels the tractor as \texttt{two-wheeled vehicle}, which exercises the
conservative \texttt{generic\_vehicle} routing instead of a committed wrong
token. (d)~TripoSR on the same object crops (6~GB cap, RTX~5090): amorphous
blobs (26{,}836 and 39{,}700 triangles); Hunyuan3D-2mini-turbo returns no
surface on either aerial crop. Neural rows are qualitative context, not a
ranking; measured IoU comparisons are the sealed synthetic wave of
...
\caption{Deployment runtime evidence, split by evidence tier. Top: measured
on real or recorded data. Bottom: packaged synthetic replays used for the
scaling limits (full run matrix in RP). Python contract timings are not
Unreal frame times; the link row is byte accounting against a modeled video
reference, not a measured radio link.}
...
SPPA-MVFit fit, median (sealed benchmark) & \(9.4\)~ms CPU per object & p95 \(10.6\)~ms \\
Fit latency on the recorded real stream (Section~\ref{sec:real-stream}) & \(11.8\)~ms median & 2--15~Hz stream \\
Measured JSON update traffic (no-ops suppressed) & 25.8--37.4~kB/s & vs modeled video \(\geq\)250~kB/s \\
\midrule
...
condition (the flight and its detections are mapped against the simulator
ground-truth anchors in RP).
...
signature on this stream. A frozen routing policy on these signals
(registered 2026-07-19 before any outcome; RP) turns the signal into a
measured coverage decision: routing detections below confidence \(0.34\) to
the nonsemantic proxy fallback lifts the stream-level median 2D IoU from
\(0.298\) (always-SPPA) to \(0.432\) (\([0.427,0.435]\)) while still
compiling the \(19\%\) most confident detections into family proxies
(always-proxy \(0.446\); oracle routing \(0.452\)). The image-overlap cost
of the prior is therefore localized where the gate fires, not uniform.
...
A 23.9-s segment of real onboard flight video (239 frames at 10~Hz,
\(1280{\times}960\), cut by the operator from the M\_20\_1RR sortie of
2026-07-06 and resynchronized to frame 752 of the original recording by
template matching, 09:39:01--09:39:25~Z) was processed with synchronized
ArduPilot telemetry (239 interpolated poses from the dataflash log, maximum
GPS gap 0.22~s). The YOLOE-26s detector reports 147 detections; tower-class
hypotheses appear in 99/239 frames (median confidence 0.31, best 0.58 at
frame 182), and the pylon's bounding box grows coherently as the drone
closes in (Figure~\ref{fig:real-video-pass}). Four lattice poles in the
area are georeferenced from PNOA orthophotos (\(\approx\)1--2~m accuracy)
as ground truth. Metric evaluation of the geo-projected footprints against
those poles is deferred: the camera mount is not calibrated (an approximate
mount projects a systematic bias), so this arm reports real detection
evidence only, not metric accuracy.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{figures/fig_real_video_pass.png}
\caption{First real-video pass (exploratory post-hoc, no metric claim).
(a)~Frame 182 of the real flight video with the measured YOLOE-26s
detection (\texttt{electric pylon}, 0.58). (b)~Real ArduPilot relative
altitude over the segment. (c)~Tower-class detections: 99 of 239 frames,
median confidence 0.31. (d)~The pylon bounding-box height over the pass;
the two arcs are the two approaches of the sortie.}
\label{fig:real-video-pass}
\end{figure}
...
detector, exact simulator GT). Read: on both oblique rings---the views a
real pass delivers---SPPA-MVFit leads every baseline, and every paired
SPPA$-$method interval is positive. Mean 3D voxel IoU against the exact tower
...
Figure~\ref{fig:e14-lidar} gives the outcome (full per-method table in RP),
and the
...
family token, and a frozen routing policy on those signals recovers the
overlap cost at \(19\%\) coverage (median \(0.432\) versus \(0.298\)
always-SPPA).
...
claim); its measured routing recovery (median \(0.432\) versus \(0.298\)
always-SPPA at \(19\%\) coverage) is likewise a single-stream policy
result.
\end{itemize}
...
once. This paper has shown that a reviewed semantic part graph is a
measurable occupancy prior for that instant per-object reconstruction. The
preregistered equal-budget comparison passes its prespecified margin on the
held-out synthetic test, the margin holds under every preregistered
observation corruption, and the decomposition attributes most of the
advantage to the family-graph representation rather than the fitter. The
same contract works where it was built to work: on a recorded real detector
stream it fits inside the stream's latency budget and provides measured
token-validation signals with a frozen routing policy that recovers the
prior's image-overlap cost at reduced coverage, and on two real flight
photos recorded for this
paper it compiles the detection end to end---a gate-clamped lattice tower at
\(5.60\times5.60\times28.00\)~m and a conservative-routed tractor---while
the measured neural generators fail with no surface or return amorphous
blobs at GPU cost on those same aerial views. The boundary of the prior is
measured with equal care, which is what makes it deployable: a wrong family
token is worse than none, adversarial violations destroy a third of the
advantage in identifiable zones, and the margin does not replicate on
external meshes at this sample size---so the contract routes, refines, or
falls back rather than hallucinating. The durable contribution is the
twin-delta update contract: family-conditioned roles, bounded in-place
updates, a compact update descriptor at telemetry-scale update traffic,
and a measured part-query value (query F1 \(0.434\) versus
\(0.087\)--\(0.145\) across occupancy and generic controls). Operation is
camera-validated today: LiDAR-class and night/fog/smoke sensing are
demonstrated in the simulated twin (E14 for camera-less sensing; E11 for
cross-view fidelity), dense packaged shape updates stay below 60~FPS, and
position is assumed given. The next steps are external public data with
metric ground truth, detector-derived silhouettes, and, only if claimed,
operator studies.
...
included in the package. A public snapshot of the package is archived at
Zenodo (DOI: \texttt{10.5281/zenodo.XXXXXXX}, pending mint at submission).
Large Unreal artifacts and historical
...
results is part of RP above and is released in the same public snapshot
(DOI above).
...
\paragraph{Ethics approval and consent to participate.} Not applicable. The
study uses synthetic benchmarks, simulator data, and field imagery collected
by the authors; no human participants or animals were involved.

\paragraph{Author contributions.} Omitted for double-anonymous review; a
contribution statement will be added in the camera-ready version.
