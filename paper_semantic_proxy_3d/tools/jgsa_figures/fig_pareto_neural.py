...
(TripoSR, Hunyuan3D-2mini-turbo, TripoSG, Hunyuan3D-2 full; oblique/mask input
conditions) on the 60-case external neural wave. Log payload axis. The point
is the modality mismatch (compact semantic descriptor vs full mesh), not a
leaderboard. TripoSG's mask-input point (IoU 0.002) is the documented
input-parity artifact of the E12 flagship wave manifest (as-is PNG input, no
RMBG/crop, parity with the other runners) and is annotated as such.

Data: benchmarks/results/sppa_neural_flagship_wave.json
...
SRC = REPO / "benchmarks" / "results" / "sppa_neural_flagship_wave.json"
...
def main() -> None:
    ...
    # (legend label, bytes, iou, color, marker, label dx, label dy, ha, note)
    points = [
        ...
        ("Hunyuan3D-2mini (mask input)", agg["hunyuan3d_2mini_turbo/mask"]["mesh_bytes"]["mean"],
         agg["hunyuan3d_2mini_turbo/mask"]["voxel_iou"]["mean"], OI["reddish_purple"], "s", 0, -18, "center", ""),
        ("Hunyuan3D-2mini (oblique input)", agg["hunyuan3d_2mini_turbo/oblique"]["mesh_bytes"]["mean"],
         agg["hunyuan3d_2mini_turbo/oblique"]["voxel_iou"]["mean"], OI["reddish_purple"], "D", -20, -14, "right", ""),
        ("TripoSG (mask input)", agg["triposg/mask"]["mesh_bytes"]["mean"],
         agg["triposg/mask"]["voxel_iou"]["mean"], OI["sky_blue"], "s", -24, 6, "right",
         " (parity artifact)"),
        ("TripoSG (oblique input)", agg["triposg/oblique"]["mesh_bytes"]["mean"],
         agg["triposg/oblique"]["voxel_iou"]["mean"], OI["sky_blue"], "D", -16, 12, "right", ""),
        ("Hunyuan3D-2 full (mask input)", agg["hunyuan3d_2_full/mask"]["mesh_bytes"]["mean"],
         agg["hunyuan3d_2_full/mask"]["voxel_iou"]["mean"], OI["black"], "s", 0, 12, "center", ""),
        ("Hunyuan3D-2 full (oblique input)", agg["hunyuan3d_2_full/oblique"]["mesh_bytes"]["mean"],
         agg["hunyuan3d_2_full/oblique"]["voxel_iou"]["mean"], OI["black"], "D", 16, 12, "left", ""),
    ]

    apply_style()
    fig, ax = plt.subplots(figsize=(6.8, 4.0))
    for label, b, iou, color, marker, dx, dy, ha, note in points:
        ax.scatter(b, iou, s=64, color=color, marker=marker, edgecolor="white",
                   linewidth=0.7, zorder=5, label=label)
        ax.annotate(f"{fmt_bytes(b)} / IoU {iou:.3f}{note}", (b, iou),
    ...
    ax.set_title("Representation cost vs fidelity, 60-case external wave — modality mismatch, not a leaderboard",
                 fontsize=8.4, pad=6)
    ax.legend(loc="upper right", fontsize=6.6, ncols=1, framealpha=0.95)
    # zone captions along the bottom edge (kept clear of points and legend)
    ax.text(1.05e3, -0.012, "descriptor payloads (≤ 33 kB)", fontsize=7.8,
            color=OI["blue"], va="bottom", ha="left")
    ax.text(2.7e8, 0.030, "neural mesh payloads (1.5–50.8 MB)", fontsize=7.8,
