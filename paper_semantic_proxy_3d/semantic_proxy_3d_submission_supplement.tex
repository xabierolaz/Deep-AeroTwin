...
\section*{Novelty in one paragraph}

The scientific novelty is \emph{not} a denser mesh, a new neural reconstructor,
or a live LLM geometry system. It is a sealed equal-budget demonstration that a
frozen semantic-family part graph is a measurable occupancy prior for lightweight
multiview proxy fitting: SPPA-MVFit versus Generic-MVFit on the same top/side
silhouettes, same five parameters, same 31-candidate budget. The Unreal
descriptor/update path is deployment context only.
...
\item All twelve family$\times$stratum cells positive (min \(\Delta=0.043\)).
\item Provenance: \texttt{synthetic\_geometry} only.
\item Protocol release: Amendment~03 local triple-role gate (not external peer review).
\item Analysis hygiene: Amendment~04 (fit grid 96; null-centered secondary \(p\)).
\end{itemize}
