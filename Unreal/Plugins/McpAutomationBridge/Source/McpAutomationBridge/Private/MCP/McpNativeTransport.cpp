	...
	FSocket* ClientSocket, const FString& SessionId, const FString& CorsOrigin)
{
	...
	if (!Arguments->HasField(TEXT("action")) && Arguments->HasField(TEXT("operation")))
	{
		FString OperationVal;
		Arguments->TryGetStringField(TEXT("operation"), OperationVal);
		Arguments->SetStringField(TEXT("action"), OperationVal);
	}
